...
impl IgvmfilegenRegister for X86Register {
    ...
    fn load_linux_direct_boot_config(
        ...
        // Import GDT.
        loader::common::import_default_gdt(importer, GDT_BASE)
            .context("importing GDT")?;
        ...
        let zero_page = loader_defs::linux::boot_params {
            hdr: loader_defs::linux::setup_header {
                ...
                ramdisk_image: (load_info
                    .initrd
                    .as_ref()
                    .map(|i| i.gpa)
                    .unwrap_or(0) as u32)
                    .into(),
                ramdisk_size: (load_info
                    .initrd
                    .as_ref()
                    .map(|i| i.size)
                    .unwrap_or(0) as u32)
        ...
        import_reg(X86Register::Cr0(
            x86defs::X64_CR0_PG | x86defs::X64_CR0_PE,
        ))?;
