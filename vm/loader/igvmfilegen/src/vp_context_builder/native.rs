...
impl VpContextBuilder for NativeVpContext {
    ...
    fn import_vp_register(&mut self, register: X86Register) {
        ...
        // For data segment registers (DS/ES/FS/GS/SS), only check for
        // duplicates against the same variant since they all map to
        // the same native context fields.
        let dominated_by_existing = match register {
            X86Register::Es(_)
            | X86Register::Fs(_)
            | X86Register::Gs(_)
            | X86Register::Ss(_) => self.registers.iter().any(|r| {
                matches!(
                    r,
                    X86Register::Ds(_)
                        | X86Register::Es(_)
        ...
        if dominated_by_existing {
            // Data segment registers all write the same native context fields.
