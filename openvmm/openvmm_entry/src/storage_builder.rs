...
impl StorageBuilder {
    ...
    pub fn build_config(
        ...
        use_legacy_pci_for_virtio_blk: bool,
    ) -> anyhow::Result<()> {
        ...
        for (i, vblk) in std::mem::take(&mut self.vtl0_virtio_blk_disks)
            ...
            .enumerate()
        {
            if use_legacy_pci_for_virtio_blk {
                // Place virtio-blk on the legacy PCI bus (for native IGVM
                // guests that don't have VMBus/VPCI).
